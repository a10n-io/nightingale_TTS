...
let refPath = "/Users/a10n/Projects/nightingale/verification_outputs/e2e_steps1_5"
let step6RefPath = "/Users/a10n/Projects/nightingale/verification_outputs/e2e_steps1_6"

print(String(repeating: "=", count: 80))
print("END-TO-END VERIFICATION: STEPS 1-6")
print(String(repeating: "=", count: 80))
print("Test text: \"\(testText)\"")
print("Random seed: 42 (locked for determinism)")
...
do {
    ...
    let step6RefURL = URL(fileURLWithPath: step6RefPath)
    ...
    // =========================================================================
    print("\n" + String(repeating: "=", count: 80))
    print("STEP 5: S3GEN EMBEDDING")
    print(String(repeating: "=", count: 80))

    // Use predicted token from Step 4 + additional test tokens
    let testSpeechTokens = MLXArray([greedyToken, Int32(3705), Int32(3706), Int32(3707), Int32(3708)]).reshaped([1, 5])
    print("Test speech tokens: \(testSpeechTokens.squeezed().asArray(Int32.self))")
    print("Test tokens shape: \(testSpeechTokens.shape)")

    // Concatenate prompt + speech tokens
    ...
    let maskBool = less(seqRange, MLXArray(fullTokenLen))
    let maskFloat = maskBool.expandedDimensions(axis: -1).asType(.float32)
    ...
    // Speaker embedding (normalize + affine)
    var soulS3Batch = soul_s3
    if soul_s3.ndim == 1 {
        soulS3Batch = soul_s3.expandedDimensions(axis: 0)
    }
    ...
    // Load Python reference
    let refFullTokens = try NPYLoader.load(contentsOf: refURL.appendingPathComponent("step5_full_tokens.npy"))
    let refMask = try NPYLoader.load(contentsOf: refURL.appendingPathComponent("step5_mask.npy"))
    let refTokenEmb = try NPYLoader.load(contentsOf: refURL.appendingPathComponent("step5_token_emb.npy"))
    let refSpkEmb = try NPYLoader.load(contentsOf: refURL.appendingPathComponent("step5_spk_emb.npy"))
    ...
    let step5Pass = fullTokensDiff < threshold && maskDiff < threshold &&
                    tokenEmbDiff < threshold && spkEmbDiff < threshold
    ...
    // Reset seed to match reference generator
    // This ensures S3Gen's internal random state matches the reference
    MLXRandom.seed(42)

    // Reload S3Gen model to match reference initialization state
    let s3genWeightsStep6 = try! MLX.loadArrays(url: s3genFP16URL)
    let vocoderWeightsStep6 = try! MLX.loadArrays(url: vocoderFP16URL)
    let s3genStep6 = S3Gen(flowWeights: s3genWeightsStep6, vocoderWeights: vocoderWeightsStep6)

    // Run encoder
    let encoderOutput = s3genStep6.encoder(tokenEmb)
    eval(encoderOutput)
    ...
    // Load Python GOLD STANDARD reference (generated by mlx-audio-plus)
    let refEncoderOutput = try NPYLoader.load(contentsOf: step6RefURL.appendingPathComponent("step6_encoder_output.npy"))
    print("\nLoaded Python GOLD STANDARD encoder output: \(refEncoderOutput.shape)")

    // Compare
    let encoderOutputDiff = maxDiff(encoderOutput, refEncoderOutput)

    print("\nComparison:")
    print("  encoder_output max_diff: \(String(format: "%.2e", encoderOutputDiff))")

    let step6Threshold: Float = 0.1  // Relaxed threshold for deep neural network
    let step6Pass = encoderOutputDiff < step6Threshold
    ...
    print("Step 5 (S3Gen Embedding): \(step5Pass ? "✅ PASSED (max_diff < \(threshold))" : "❌ FAILED (max_diff >= \(threshold))")")
    print("Step 6 (S3Gen Encoder): \(step6Pass ? "✅ PASSED (max_diff < \(step6Threshold))" : "❌ FAILED (max_diff >= \(step6Threshold))")")

    let allPass = step1Pass && step2Pass && step3Pass && step4Pass && step5Pass && step6Pass
