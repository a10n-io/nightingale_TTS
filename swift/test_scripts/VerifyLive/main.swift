...
let VERIFY_PATH = "\(PROJECT_ROOT)/verification_outputs/live"
...
func loadConfig() throws -> (text: String, voice: String) {
    let configURL = URL(fileURLWithPath: "\(VERIFY_PATH)/config.json")
...
do {
    // Load config from Python run
    let (text, voiceName) = try loadConfig()
    print("Text: \"\(text)\"")
    print("Voice: \(voiceName)")
    print()

    let modelsURL = URL(fileURLWithPath: MODELS_PATH)
    let voiceURL = URL(fileURLWithPath: "\(VOICES_PATH)/\(voiceName)/npy")
    let verifyURL = URL(fileURLWithPath: VERIFY_PATH)
    ...
    let allPass = step1Pass && step2Pass
