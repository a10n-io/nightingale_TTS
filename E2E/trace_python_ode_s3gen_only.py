#!/usr/bin/env python3
"""Trace Python ODE to see intermediate values (S3Gen only)."""
...
print("Loading S3Gen...")
s3gen = S3Gen.from_local(str(model_dir), device=device)

print("Loading voice conditionals...")
voice_path = PROJECT_ROOT / "baked_voices" / "samantha" / "baked_voice.pt"
conds = Conditionals.load(str(voice_path), map_location=device)
